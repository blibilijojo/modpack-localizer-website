...
import { allTexts } from "../content.ts";
...
export default function App({ downloadUrl, version }: AppProps) {
  const lang = signal<"zh" | "en">("zh");
  const content = computed(() => allTexts[lang.value]);

  return (
    <>
      <Head>
        <title>{content.value.title}</title>
        <meta name="description" content={content.value.description} />
      </Head>
      <div class="bg-gray-900 text-gray-200 min-h-screen font-sans">
        ...
        <main class="container mx-auto px-6 py-12">
          <section class="text-center">
            ...
            <div class="mt-8">
              <a href={downloadUrl!} class="inline-block bg-blue-600 text-white font-bold text-lg px-8 py-3 rounded-lg shadow-lg hover:bg-blue-700 transition-transform transform hover:scale-105">
                {content.value.latest_release} {version && `(${version})`}
              </a>
            </div>
            <div class="mt-12">
          ...
          <section class="mt-20">
              <h3 class="text-3xl font-bold text-center mb-10">{content.value.feature_showcase}</h3>
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                  <div class="bg-gray-800 p-4 rounded-lg">
                      <h4 class="text-xl font-semibold mb-2 text-center">{content.value.manual_workbench}</h4>
                      <p class="text-sm text-gray-400 mb-4 text-center">{content.value.manual_workbench_desc}</p>
                  ...
                  <div class="bg-gray-800 p-4 rounded-lg">
                      <h4 class="text-xl font-semibold mb-2 text-center">{content.value.dict_search}</h4>
                      <p class="text-sm text-gray-400 mb-4 text-center">{content.value.dict_search_desc}</p>
          ...
          <section class="mt-20">
              <h3 class="text-3xl font-bold text-center mb-10">{content.value.features_title}</h3>
